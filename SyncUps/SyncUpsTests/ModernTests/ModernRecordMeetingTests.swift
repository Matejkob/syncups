...
struct ModernRecordMeetingTests {
  @Test
  ...
  func authorization() async {
    await confirmation { onMeetingFinishedConfirmation in
      await confirmation(expectedCount: 0) { startTaskConfirmation in
        let model = withDependencies {
          $0.continuousClock = ImmediateClock()
          $0.soundEffectClient = .noop
          $0.speechClient.requestAuthorization = { .denied }
          $0.speechClient.authorizationStatus = { .notDetermined }
          $0.speechClient.startTask = { _ in
            startTaskConfirmation()
            return .never
          }
        } operation: {
          RecordMeetingModel(
